...
const Num = ({ label, value, onChange }: { label: string; value: number; onChange: (n: number) => void }) => (
  <label style={{ flex: 1, minWidth: 0 }}>
    ...
    <input type="number" value={Math.round(value)} onChange={(e) => onChange(Number(e.target.value) || 0)} style={inputStyle} />
  </label>
);
...
export const TextPanel = ({
  ...
  const set = (id: string, next: Partial<SlideElement>) =>
    onChange({ ...slide, elements: slide.elements.map((e) => (e.id === id ? ({ ...e, ...next } as SlideElement) : e)) });
  ...
  return (
    <div style={{ width: 340, flex: "0 0 auto", overflowY: "auto", padding: 20, background: "#0d0f16", borderLeft: "1px solid rgba(255,255,255,0.06)", color: "#e8e9ee" }}>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        <input type="number" value={slide.durationInFrames} onChange={(e) => onChange({ ...slide, durationInFrames: Math.max(1, Number(e.target.value) || 1) })} style={inputStyle} />
      </div>
      ...
      {sel && (
        <div style={{ marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <Num label="X" value={sel.x} onChange={(x) => set(sel.id, { x })} />
            <Num label="Y" value={sel.y} onChange={(y) => set(sel.id, { y })} />
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <Num label="W" value={sel.w} onChange={(w) => set(sel.id, { w })} />
            <Num label="H" value={sel.h} onChange={(h) => set(sel.id, { h })} />
          </div>

          {sel.type === "text" && (
            <>
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <Num label="Font size" value={sel.style?.fontSize ?? 40} onChange={(fontSize) => setStyle(sel.id, { fontSize }, sel.style)} />
                <Num label="Weight" value={sel.style?.fontWeight ?? 400} onChange={(fontWeight) => setStyle(sel.id, { fontWeight }, sel.style)} />
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={labelStyle}>Align</div>
                <div style={{ display: "flex", gap: 6 }}>
                  {(["left", "center", "right"] as const).map((a) => (
                    <button key={a} onClick={() => setStyle(sel.id, { align: a }, sel.style)} style={{ ...inputStyle, cursor: "pointer", background: (sel.style?.align ?? "left") === a ? "#2a2d3d" : "#15171f" }}>{a}</button>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: 4 }}>
                <div style={labelStyle}>Color</div>
                <input value={sel.style?.color ?? ""} placeholder="#f5f6fa" onChange={(e) => setStyle(sel.id, { color: e.target.value }, sel.style)} style={inputStyle} />
              </div>
            </>
          )}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <label style={{ flex: 1 }}>
              ...
              <select value={sel.animation?.preset ?? "none"} onChange={(e) => set(sel.id, { animation: { preset: e.target.value as AnimationPreset, start: sel.animation?.start ?? 0 } })} style={inputStyle}>
                {PRESETS.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
            </label>
            <Num label="Start" value={sel.animation?.start ?? 0} onChange={(start) => set(sel.id, { animation: { preset: sel.animation?.preset ?? "none", start } })} />
          </div>
        </div>
      )}
